...
const ARRAY_FRASE = {
  0: {
    ...
    color: "red",
    description:
      "La <strong><u>flexibilitat</u></strong> és la capacitat que té un alumne en treballar amb una visió global del projecte, tant del producte/servei com de l'aprenentatge. Això permet fer adaptacions fàcilment.",
  },
  1: {
    name: "RESPONSABILITAT",
    color: "orange",
    description:
      "La <strong><u>responsabilitat</u></strong> és la capacitat en que un alumne coneix els objectius amb precisió i es treballa de manera continuada i intensiva.",
  },
  2: {
    name: "AUTONOMIA",
    color: "purple",
  ...
  3: {
    ...
    color: "green",
    description:
      "La <strong><u>sociabilitat</u></strong> és la capacitat en que un alumne es comunica intensivament tant en el temps i espai acadèmic com a fora d'ell.",
  },
  4: {
    name: "EVOLUCIO",
    color: "cyan",
    description:
      "La <strong><u>evolució</u></strong> és la capacitat en que un alumne treballa de manera constant, completant els fulls de ruta amb escrit crític. Hi ha preocupació entrenant el sentit de cada tasca i com connecta amb el global del projecte.",
  },
};
...
function create_mobile_controllers() {
  ...
  buttonsConfig.forEach(({ id, text }) => {
    const button = document.createElement("button");
    button.innerText = text;
    button.id = id;
    button.style.width = id === "shoot" ? "auto" : "50px";
  ...
  // Add controller to the board element
  BOARD_GAME_ELEMENT.appendChild(controller);
  ...
  function move(direction) {
    if (ship && ball) {
      ship.directionX = direction;
      ship.update();

      if (ball.speedX === 0 && ball.speedY === 0 && ship.isMove) {
        ball.x += direction * SPEED_DEFAULT_SHIP; // Use direction to move left or right
        ball.update();
      }
    }
  }
  ...
  function shoot(direction) {
    if (ship && ball && ball.speedX === 0 && ball.speedY === 0) {
      ship.set_arrow_direction(direction);
    }
  }
  ...
  function shootBall() {
    if (ship && ball) {
      if (ball.speedX === 0 && ball.speedY === 0) {
        ...
        ball.interval_ball = setInterval(() => {
          ball.update();
        }, ball.time_update);
      }
    }
  ...
  // Set up timers for button presses
  const timers = {};
  const buttonStates = {
    left: false,
    right: false,
  ...
  ["left", "right", "shoot", "shoot_left", "shoot_right"].forEach((button) => {
    const btnElement = document.getElementById(button);

    const startAction = (action) => {
      if (buttonStates[button]) return; // Prevent multiple starts
      buttonStates[button] = true;
      action(); // Execute immediately

      // Start a timer for continuous action
      timers[button] = setInterval(action, TIME_REPEAT_CONTROLLER);
    };

    const stopAction = () => {
      buttonStates[button] = false;
      clearInterval(timers[button]);
      delete timers[button];
    };

    // On touch start / mouse down
    btnElement.addEventListener("touchstart", (event) => {
      event.preventDefault(); // Prevent default behavior
      startAction(button === "left" ? () => move(-1) : 
                   button === "right" ? () => move(1) : 
                   button === "shoot" ? shootBall : 
                   button === "shoot_left" ? () => shoot(-1) : 
                   () => shoot(1));
    });
    ...
    // On touch end / mouse up
    btnElement.addEventListener("touchend", stopAction);
    btnElement.addEventListener("mouseup", stopAction);
  });
}
...
function win() {
  ...
  if (true) {
    ...
    content_frase.querySelectorAll("span").forEach((element) => {
      element.addEventListener("mouseover", (e) => {
        if (!element.firstElementChild) {
          ...
          ballon_message.style.top = `-${
            ballon_message.getBoundingClientRect().height
          }px`;
        }
      });
    ...
    send_score(ship.points, (data, previous_score) => {
      ...
      BOARD_GAME_FINISH_ELEMENT.querySelector(
        "#points-game-finish"
      ).innerText = `${previous_score} punts`;
      BOARD_GAME_FINISH_ELEMENT.querySelectorAll(
        "#content-game-finish > div"
      ).forEach((element, index) =>
        setTimeout(() => (element.style.visibility = "visible"), 1000 * index)
      );
...
function game_over() {
  ...
  if (timer.minutes < 0 && ARRAY_BLOCKS_REMAINING.length !== 0) {
    ...
    send_score(ship.points, (data, previous_score) => {
      //Show game over menu.
      BOARD_GAME_FINISH_ELEMENT.querySelector("#text-game-finish").innerHTML =
        "GAME OVER <br/><br/>Que pena s'ha acabat el temps, no has pogut descobrir el significat de la FRASE, torna-ho a intentar!";
      BOARD_GAME_FINISH_ELEMENT.querySelector(
        "#points-game-finish"
      ).innerText = `${previous_score} punts`;
      BOARD_GAME_FINISH_ELEMENT.querySelectorAll(
...
function send_score(points, callback) {
  ...
  if (
    Object.values(ARRAY_BLOCKS_REMAINING).length === 0 ||
    (timer.minutes < 0 && ARRAY_BLOCKS_REMAINING.length !== 0)
  ) {
